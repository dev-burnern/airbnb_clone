...
export default function HeaderSearchBar() {
  ...
  const DatesDropdown = () => (
      // 기존: left-1/4 -translate-x-1/2
      // 수정: left-1/2 -translate-x-1/2 -> 상위 요소 기준으로 정확히 중앙에 위치
      <div className="absolute top-full mt-6 left-1/2 -translate-x-1/2 w-[750px] bg-white rounded-3xl shadow-2xl p-6 z-10 border border-gray-100">
  ...
  const CalendarMonth: React.FC<CalendarMonthProps> = ({ monthData, dates, onDateSelect }) => {
    ...
    const isCheckOut = (day: number) => {
        if (!day || !checkOutDate) return false;
        const dateStr = formatDate(new Date(year, month, day));
            ...
            <div className="grid grid-cols-7 gap-y-1">
                ...
                {calendarDays.map((day, index) => {
                    ...
                    if (disabled) {
                        ...
                        // 범위가 선택되었을 때 인접한 셀의 배경을 채우기 위한 특수 스타일
                        if (isStart && inRange) cellClasses += ' rounded-l-none';
                        if (isEnd && inRange) cellClasses += ' rounded-r-none';
                        if (isStart && isEnd) cellClasses = cellClasses.replace('rounded-l-none', '').replace('rounded-r-none', '');
                    } else if (inRange) {
                    ...
                    const wrapperClasses = inRange && !isStart && !isEnd ? 'bg-gray-100' : '';

                    return (
                        <div key={index} className={`relative ${wrapperClasses}`}>
                            <div
  ...
  return (
    <div className="relative" ref={searchBarRef}>
      <div 
        className="flex items-center bg-white rounded-full shadow-md hover:shadow-lg transition w-[850px] p-2"
      >
        {/* 여행지 필드 (flex-1 적용) */}
        <div 
        ...
        {/* 여행자 필드 및 검색 버튼 통합 (래퍼에 flex-1 적용) */}
        <div 
          className={`flex items-center px-2 py-2 rounded-full cursor-pointer flex-1 ${
